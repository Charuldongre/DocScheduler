...
const Appointment = () => {
  ...
  const appointments = useSelector((state) => state.appointments.appointments); // Fetch appointments
  ...
  const handleBookAppointment = () => {
    if (selectedDate && slotTime) {
      ...
      if (isAlreadyBooked) {
        toast.warning("You have already booked an appointment for this doctor at the same time.", {
      ...
      // Proceed with booking since it's a new appointment
      const appointmentData = {
        doctorId: docId,
        name: docInfo.name,
        image: docInfo.image,
        speciality: docInfo.speciality,
        date: formattedDate, // Store date in consistent format
        time: slotTime,
      };
    ...
    docInfo && (
      <div>
        <div className="flex flex-col sm:flex-row gap-4">
          <div>
            <img
              className="bg-[#5f6ff] w-full sm:max-w-72 rounded-lg"
              src={docInfo.image}
              alt=""
            />
          </div>
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
          ...
          <div className="flex gap-3 items-center w-full overflow-x-scroll mt-4">
            {docSlots.length > 0 &&
              docSlots
                .filter((daySlots) => daySlots.length > 0) // Hide empty slot days
                .map((daySlots, index) => (
                  <div
