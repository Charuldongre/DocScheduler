    ...
    <div className="flex flex-col items-center gap-4 py-16 text-gray-800" id="speciality">
        ...
        <div className='flex sm:justify-center gap-4 pt-5 w-full overflow-x-scroll'>
            {specialityData.map((item,index)=>(
                <Link onClick={()=>scrollTo(0,0)} className="flex flex-col items-center text-xs flex-shrink-0 cursor-pointer hover:translate-y-[-10px] transition-all duration-500" to={`/doctors/${item.speciality}`} key={index}>
